...
private:
  const DeclRefExpr * unwrapCallbackArg(const Expr* callbackArg);
  void getCallbackDefinition(const Expr* callbackSignature);
  void getCallbackMemberRefs(const Expr* callbackBody);

    void getCallbackDefinition(const DeclRefExpr *CallbackSignature);
};
