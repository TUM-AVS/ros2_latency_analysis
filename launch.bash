...
pids=()
(
    ssh "$ssh_id" "${sim_username}"@"${sim_hostname}" "rm -f ${sim_rootdir}/scenario_runner/worker.log" &&
    ...
    ssh "$ssh_id" "${sim_username}"@"${sim_hostname}" "pkill --signal SIGKILL -f 'AWSIM|sim_orchestrator'"
) &
pids+=($!)
echo "[LAUNCHER] Launched sim worker on ${sim_username}@${sim_hostname}"
(
    ssh "$ssh_id" "${aw_username}"@"${aw_hostname}" "rm -f ${aw_rootdir}/scenario_runner/worker.log" &&
    ...
    ssh "$ssh_id" "${aw_username}"@"${aw_hostname}" "pkill --signal SIGKILL -f 'ros|http.server|aw_orchestrator'"
) &
pids+=($!)
